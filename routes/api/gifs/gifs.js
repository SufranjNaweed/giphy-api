...
router.get('/',  async (req, res) => {
    const API_URL = `${base_url_api}gifs/trending?&api_key=${api_key}`;
...
router.get('/search/:wordSearch', async (req, res)=>{
    ...
    const API_URL = `${base_url_api}gifs/search?q=${search}&api_key=${api_key}`;
    const data = await tools.fetcher(API_URL);
    if(data)
        return res.status(200).send(data);
    else
...
router.get('/random', async(req, res) =>{
    const API_URL = `${base_url_api}gifs/random?&api_key=${api_key}`;
    const data = await tools.fetcher(API_URL);
    if(data)
        return res.status(200).send(data);
    else
...
router.get('/search/byID/:idSearch', async (req, res)=>{
    ...
    const API_URL = `${base_url_api}gifs/${id}/&api_key=${api_key}`;
    console.log(API_URL);
    const data = await tools.fetcher(API_URL);
    if(data)
        return res.status(200).send(data);
